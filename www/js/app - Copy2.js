...
imageApp.controller("SecureController", function($scope, $ionicHistory, $firebaseArray, $cordovaCamera, $ionicLoading, $state, $rootScope,
  $ionicModal,  $window, $cordovaGeolocation, $ionicPopup) {
...
var confirmPopup;
 ...
 showConfirm();
    ...
    var location;
     ...
     /*
     cordova.plugins.diagnostic.isLocationAvailable(function(message) {alert("loc  available")}, function(error) {
        //alert(error);
        $ionicLoading.show({ template: "Enable location setting.", noBackdrop: true, duration: 2000 });
    ...
    $scope.upload = function() {
     // alert("Going to capture image.");
        ...
        $cordovaCamera.getPicture(options).then(function(imageData) {
            syncArray.$add({address: $scope.address, image: imageData, timestamp: Firebase.ServerValue.TIMESTAMP}).then(function() {
